...
int object_write(ObjectType type, const void *data, size_t len, ObjectID *id_out) {
    ...
    if (write(fd, obj, total) != (ssize_t)total) { close(fd); free(obj); return -1; }
    fsync(fd);
    close(fd);
    free(obj);
...
int object_read(const ObjectID *id, ObjectType *type_out, void **data_out, size_t *len_out) {
    ...
    if (fread(buf, 1, total, f) != total) { fclose(f); free(buf); return -1; }
    fclose(f);

    // Step 2
    ObjectID recomputed;
    compute_hash(buf, total, &recomputed);
    ...
    *len_out = (size_t)(buf + total - data_start);
